...
for (let i = 0; i < 3; i++) {
    links=prompt('create a name of links(www.google.com)')
    document.querySelector(".links").innerHTML+=`<a href="//${links}"  target='blank' >${links}</a><br>`;
    document.querySelectorAll(".links a")[i].style.color=prompt('create a color of links')
}

function f(){
    document.querySelector(".wrapper").style.background=prompt("background color:(gold)")
    document.querySelector(".wrapper").style.fontFamily=prompt("fontFamily of all page(cursive)")
    document.querySelector("h1").style.textAlign=prompt("textAlign for Header(center)")
    let list=prompt("ul or ol")
    document.querySelector(".list").innerHTML=`<${list}><li >gav</li><li>miaw</li>
    <li>krya</li>  <${list}/>`
    document.querySelector(".wrapper").style.fontStyle=prompt("fontStyle: (italic)")
    document.querySelector("p").style.fontSize=prompt("fontsize:  (20px)")
    
    if(list=="ul"){
        document.querySelector("ul").style.listStyleType=prompt("listStyleType  (circle)")
    }
    else{
        document.querySelector("ol").style.listStyleType=prompt("listStyleType  (armenian)")
    }
}
f()
